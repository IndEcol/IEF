...
if Name_Script != 'EB3Parse_Capital': # Name of this script must equal the specified name in the Excel config file
...
DES_ProductNames163_original_Cap = DES_ProductNames163_original.copy()
DES_ProductNames163_original_Cap.append('Fixed aggregate capital (augmentation)')
...
for m in eval(ScriptConfig['YearSelect']):
    CurrentYear = m
    print(CurrentYear)
    ...
    FixedCapitalConsumption     = MRIO_F[8,:].copy()
    ...
    MRIO_A_Cap = np.zeros((MRIO_Z.shape[0] + DES_NoofCountries, MRIO_Z.shape[1] + DES_NoofCountries))
    for m in range(0,DES_NoofCountries):
    ...
    Filestring_Matlab_out = ScriptConfig['IOModelPath'] + str(CurrentYear) + '_ITC_Capital.mat'
    scipy.io.savemat(Filestring_Matlab_out, mdict={'ScriptConfig':ScriptConfig,
                                                        ...
                                                        'EB3_ProductNames163_Cap':DES_ProductNames163_original_Cap,
